...
const fetchProducts = async () => {
  try {
    ...
    products.value = await $fetch(
      'http://127.0.0.1:8000/api/products?category=accessories'
    )
  } catch (error) {
    console.error(error)
  } finally {
...
<template>
  <div class="min-h-screen bg-slate-50 text-slate-900">
    <section class="bg-gradient-to-r from-slate-900 via-purple-800 to-pink-600 text-white py-20 px-6">
    ...
    <section class="max-w-7xl mx-auto px-6 py-10">
      <div class="flex items-center gap-2 text-sm text-slate-500 mb-8">
        ...
        <NuxtLink to="/accessories" class="hover:text-purple-600">
          Accessories
        </NuxtLink>
      ...
      <div v-else-if="filteredProducts.length === 0" class="py-20 text-center text-slate-600">
        No products found. Add accessories products from Admin Products.
      </div>
        ...
        >
          ...
          <div class="p-6">
            ...
            <p class="text-slate-600 leading-7 mt-4 line-clamp-3">
              {{ product.description || 'Premium quality product from QuiCart.' }}
            </p>
  ...
  <button
    ...
    class="w-full bg-slate-900 text-white py-3 rounded-full font-bold hover:bg-purple-700 transition disabled:opacity-50"
  >
